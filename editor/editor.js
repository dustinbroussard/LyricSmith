document.addEventListener('DOMContentLoaded', () => {
    const app = {
        ...
        exitPerformanceBtn: document.getElementById('exit-editor-btn'),
        ...
        // Load editor state from query parameters
        loadPerformanceState() {
            const params = new URLSearchParams(window.location.search);
            this.editorSetlistId = params.get('setlistId') || null;
            const songId = params.get('songId');
            if (this.editorSetlistId) {
                const setlistRaw = localStorage.getItem('setlists');
                if (setlistRaw) {
                    const setlists = JSON.parse(setlistRaw);
                    const setlist = setlists.find(s => s.id === this.editorSetlistId);
                    if (setlist) {
                        this.editorSongs = setlist.songs
            ...
            this.currentPerformanceSongIndex = songId
                ? this.editorSongs.findIndex(s => s.id === songId)
                : 0;
        ...
        maybeResumeSetlist() {
            ...
            if (
                lastPerf &&
                lastPerf.setlistId &&
                lastPerf.setlistId === this.editorSetlistId &&
                typeof lastPerf.songIndex === "number" &&
                lastPerf.songIndex > 0 &&
                this.editorSongs[lastPerf.songIndex]
            ) {
                const resume = confirm(
                    "Resume this setlist where we left off? (Song " +
                    (lastPerf.songIndex + 1) +
                    ": " +
                    (this.editorSongs[lastPerf.songIndex]?.title || "Unknown") +
        ...
        displayCurrentPerformanceSong() {
            ...
            const totalSongs = this.editorSongs.length;
	...
	adjustFontSize(amount) {
        ...
        navigatePerformanceSong(direction) {
            ...
            if (newIndex >= 0 && newIndex < this.editorSongs.length) {
        ...
        exitPerformanceMode() {
            const perf = {
                setlistId: this.editorSetlistId || null,
